...
| 段階 | 状態 |
|---|---|
| **守**（2026-05-02 完成） | gstack を subtree で取り込み、Type 1 として翻訳・試用を蓄積。runtime + 30 skill（実翻訳）+ Phase 6 予約スタブ 10 件 = top-level 40 directory が揃った |
| **破**（現在） | 個別スキルが Type 2 → Type 3 の進化を繰り返す |
| **離** | gstack 由来（Type 1）が全て Type 3 化 or 削除された独立期 |

**Phase 3 + Phase 3.5 完遂（2026-05-02）**: runtime（bin 約 50 個 + テンプレート機構 + voice 翻案ガイドライン v1/v2 + ETHOS.md）と Type 1 実翻訳 25 skill（既存 5 と合わせて 30 skill）+ Phase 6 予約スタブ 10 件が揃い、「守」が完成。Phase 4（連鎖機構）/ Phase 5（記憶機構）に進む段階に到達。詳細は [CONTRIBUTING.md の Phase 進捗](CONTRIBUTING.md#phase-進捗2026-05-02-時点phase-3--phase-35-totally-done) 参照。
