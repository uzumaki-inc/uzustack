...
version: 0.0.0
description: |
  QA テスト + サイト dogfood 用の高速 headless browser。任意の URL に
  navigate、要素操作、ページ状態の確認、操作前後の差分、注釈付き
  screenshot、レスポンシブレイアウト確認、フォーム / アップロード /
  ダイアログのテスト、要素状態 assertion を実行する。1 コマンド ~100ms。
  機能テスト、デプロイ確認、ユーザーフロー dogfood、エビデンス付き
  バグ起票が必要なときに使用する。「ブラウザで開いて」「サイトをテスト」
  「screenshot を撮って」「dogfood」と要求されたときに使用する。
triggers:
  - ブラウザでページを開く
  - headless browser
  - ページの screenshot を撮る
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"browse","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.uzustack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/uzustack/bin/uzustack-timeline-log '{"skill":"browse","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
