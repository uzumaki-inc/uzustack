...
export const RESOLVERS: Record<string, ResolverFn> = {
  ...
  // Phase 4+ で `eval "$(uzustack-slug)"` の single-line bash を返す予定
  SLUG_EVAL: (_ctx, _args) => '',
  // Phase 4+ で benefits-from frontmatter を skill 推奨文に変換する予定
  BENEFITS_FROM: (_ctx, _args) => '',
  // Phase 4+ で別 skill を inline で起動する指示文を返す予定（{{INVOKE_SKILL:<skill-name>}}）
  INVOKE_SKILL: (_ctx, _args) => '',
  // Phase 4+ で spec review loop の指示文を返す予定（plan 系 skill）
  SPEC_REVIEW_LOOP: (_ctx, _args) => '',
  // Phase 4+ で codex 経由の plan 外部視点 review 指示文を返す予定
  CODEX_PLAN_REVIEW: (_ctx, _args) => '',
  // Phase 4+ で review readiness dashboard 表示指示文を返す予定
  REVIEW_DASHBOARD: (_ctx, _args) => '',
  // Phase 4+ で plan file への review report 追記指示文を返す予定
