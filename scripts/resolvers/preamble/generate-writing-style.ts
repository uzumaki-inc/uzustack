...
export function generateWritingStyle(_ctx: TemplateContext): string {
  ...
  const jargonBlock = terms.length > 0
    ...
    : `**用語リスト：**（未ロード — \`scripts/jargon-list.json\` が見つからないか不正な形式）。リストが復元されるまで用語 gloss ルールをスキップ。`;

  return `## Writing Style（preamble echo に \`EXPLAIN_LEVEL: terse\` が表示されている場合、またはユーザーの現在のメッセージが明示的に terse / no-explanations 出力を要求している場合、この section 全体をスキップ）
