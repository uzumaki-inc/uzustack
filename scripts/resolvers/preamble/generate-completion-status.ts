 ...
 *
 * preamble の先頭（position 1）に配置。モデルが他の指示より先に
 * plan-mode の権威的ルールを読めるようにする。
 *
 * Composition position: index 1 in scripts/resolvers/preamble.ts —
 * generatePreambleBash の後（_SESSION_ID / _BRANCH / _TEL env vars が
 * plan-mode-aware telemetry の前に存在する）、generateUpgradeCheck +
 * onboarding gates の前。
 */
...
\`\`\`bash
${ctx.paths.binDir}/uzustack-learnings-log '{"skill":"SKILL_NAME","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"observed"}'
\`\`\`
