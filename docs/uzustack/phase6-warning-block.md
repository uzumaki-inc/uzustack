...
warning block 自体は uzustack voice (= 経営者文脈 + 日本語) で書く。 ただし browse 機構の英語 error / output / Chromium binary 出力 自体は Phase 6 まで翻案保留 (= 英語のまま動作させる) と articulate する (詳細は `docs/uzustack/translation-voice-guide.md` の「Phase 6 待ち skill の voice 翻案射程」 section)。
